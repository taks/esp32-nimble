...
pub fn return_code_to_string(rc: i32) -> Option<&'static str> {
    ...
    // ATT errors (BLE_HS_ERR_ATT_BASE : 0x100)
        ...
        match rc - sys::BLE_HS_ERR_ATT_BASE {
            ...
            sys::BLE_ATT_ERR_PREPARE_QUEUE_FULL => {
                Some("Too many prepare writes have been queued.")
            }
            ...
            sys::BLE_ATT_ERR_INSUFFICIENT_RES => {
                Some("Insufficient Resources to complete the request.")
            }
            _ => None,
        }
    }
    // HCI errors (BLE_HS_ERR_HCI_BASE : 0x200)
        ...
        match rc - sys::BLE_HS_ERR_HCI_BASE {
            ...
            sys::ble_error_codes_BLE_ERR_INV_HCI_CMD_PARMS => {
                Some("Invalid HCI Command Parameters")
            }
            sys::ble_error_codes_BLE_ERR_REM_USER_CONN_TERM => {
                Some("Remote User Terminated Connection")
            }
            sys::ble_error_codes_BLE_ERR_CONN_TERM_LOCAL => {
                Some("Connection Terminated By Local Host")
            }
            sys::ble_error_codes_BLE_ERR_CONN_TERM_MIC => {
                Some("Connection Terminated; MIC Failure")
            }
